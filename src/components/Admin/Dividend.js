...
class Dividend extends Component {
  ...
  onChanger1(e) {
    ...
    var sry = sel.split('-')[1];
    $.ajax({
       url: "http://localhost:3001/api/getdividend",
       type: "POST",
       data:{fromyear:fry,toyear:sry},
  ...
  changeApplicant = (e) =>{
    ...
    var yer =  document.getElementById("finanyear").value
     ...
     $.ajax({
      ...
      type: "POST",
      data:{fromyear:fry,toyear:sry,pan:e.target.value},
...
changeyear = (e) =>{
  ...
  if(this.state.rvalue === "yes"){
    ...
    $.ajax({
      url: "http://localhost:3001/api/getdividenduserwise",
      type: "POST",
      data:{fromyear:fry,toyear:sry,pan:user},
       success: function (res3) {
        this.setState({
          data3: res3.data,
          msg3: res3.message});
